      ...
      <Form onSubmit={this.handleSubmit}>
        ...
        <FormItem
          ...
          label={(
            <span>
              内存使用上限&nbsp;
              <Tooltip title="设定用户内存（包括文件缓存）的最大用量。如果没有指定单位，则该数值将被解读为字节。但是可以使用后缀代表更大的单位 —— k 或者 K 代表千字节，m 或者 M 代表兆字节 ，g 或者 G 代表千兆字节。-1 可以移除全部已有限制。">
        ...
        >
          {getFieldDecorator('limit', {
            ...
            <Input placeholder={'设定用户内存的最大用量, 如：20M 或 100M, 使用-1解除限制'}/>,
          )}
        </FormItem>
        <FormItem
          {...formItemLayout}
          label={(
            <span>
              OOM处理&nbsp;
              <Tooltip
                title="内存不足时的处理， 0：杀死程序，1：进行自动扩充">
                <Icon type="question-circle-o"/>
              </Tooltip>
            </span>
          )}
        >
          {getFieldDecorator('oom_control', {
            rules: [{
              type: 'string',
            }, { required: false }],
          })(
            <Input placeholder={'填入 0 或 1，0：杀死程序，1：暂停程序'}/>,
          )}
        </FormItem>
        {/*<FormItem*/}
          {/*{...formItemLayout}*/}
